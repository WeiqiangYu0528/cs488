...
void A0::guiLogic()
{
	...
	ImGui::Begin("Shape Properties", &showDebugWindow, ImVec2(100,100), opacity,
			windowFlags);
		// Create Quit Button, and check if it was clicked:
		if( ImGui::Button( "Quit Application" ) ) {
			glfwSetWindowShouldClose(m_window, GL_TRUE);
		}
		// Create Reset Button, and check if it was clicked:
...
bool A0::keyInputEvent(int key, int action, int mods) {
	...
	if (action == GLFW_PRESS) {
		if (key == GLFW_KEY_EQUAL) {
			...
			m_shape_size = glm::clamp(m_shape_size * factor, 0.0f, 10.0f);
		...
		if (key == GLFW_KEY_LEFT || key == GLFW_KEY_A) {
			m_shape_translation.x -= 0.1f;
		}
		if (key == GLFW_KEY_RIGHT || key == GLFW_KEY_D) {
			m_shape_translation.x += 0.1f;
		}
		if (key == GLFW_KEY_UP || key == GLFW_KEY_W) {
			m_shape_translation.y += 0.1f;
		}
		if (key == GLFW_KEY_DOWN || key == GLFW_KEY_S) {
			m_shape_translation.y -= 0.1f;
		}
		if (key == GLFW_KEY_SPACE) {
			m_shape_rotation = (m_shape_rotation + PI / 10);
			if (m_shape_rotation >= 2 * PI) {
				m_shape_rotation = 0.0f;
			}
		}
	}
