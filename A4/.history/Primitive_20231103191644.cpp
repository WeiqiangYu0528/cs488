...
bool NonhierBox::intersect(Ray& ray, IntersectionData& data) {
    ...
    if (t_min == (minBound.x - ray.origin.x) / ray.direction.x) {
        normal = glm::vec3(-1.0f, 0.0f, 0.0f); // Left face
    } else if (t_min == (maxBound.x - ray.origin.x) / ray.direction.x) {
        normal = glm::vec3(1.0f, 0.0f, 0.0f); // Right face
    } else if (t_min == (minBound.y - ray.origin.y) / ray.direction.y) {
        normal = glm::vec3(0.0f, -1.0f, 0.0f); // Bottom face
    } else if (t_min == (maxBound.y - ray.origin.y) / ray.direction.y) {
        normal = glm::vec3(0.0f, 1.0f, 0.0f); // Top face
    } else if (t_min == (minBound.z - ray.origin.z) / ray.direction.z) {
        normal = glm::vec3(0.0f, 0.0f, -1.0f); // Back face
    } else if (tMin == (maxBound.z - ray.origin.z) / ray.direction.z) {
