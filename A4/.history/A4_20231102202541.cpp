  ...
  std::cout << "F23: Calling A4_Render(\n" <<
	...
	for (uint y = 0; y < h; ++y) {
		for (uint x = 0; x < w; ++x) {
			...
			glm::vec3 color(backgroundImage(x, y, 0), backgroundImage(x, y, 1), backgroundImage(x, y, 2));
			if (root->intersect(ray, data)) {
				color = getColor(root, eye, data, ambient, lights);
			}
			// Red: 
...
glm::vec3 getColor(SceneNode * root, const glm::vec3 & eye, IntersectionData& data, const glm::vec3 & ambient, const std::list<Light *> & lights) {
	glm::vec3 color(0.0f);
