...
int gr_nh_cone_cmd(lua_State* L)
{
  ...
  data->node = new GeometryNode(name, new NonhierCone(height));
...
int gr_nh_plane_cmd(lua_State* L)
{
  ...
  data->node = new GeometryNode(name, new NonhierPlane(pos, normal, size));
...
int gr_light_cmd(lua_State* L)
{
  ...
  glm::vec3 pos, u_vec, v_vec, w_vec, color, falloff;

  get_tuple(L, 1, &pos[0], 3);
  get_tuple(L, 2, &u_vec[0], 3);
  get_tuple(L, 3, &v_vec[0], 3);
  get_tuple(L, 4, &w_vec[0], 3);
  size_t u_steps = luaL_checknumber(L, 5);
  size_t v_steps = luaL_checknumber(L, 6);
  size_t w_steps = luaL_checknumber(L, 7);
  get_tuple(L, 8, &color[0], 3);
  get_tuple(L, 9, &falloff[0], 3);

  data->light = new AreaLight(pos, u_vec, v_vec, w_vec, u_steps, v_steps, w_steps, color, falloff); 
...
int gr_material_cmd(lua_State* L)
{
  ...
  data->material = new PhongMaterial(glm::vec3(ka[0], ka[1], ka[2]),
...
int gr_node_set_reflection_cmd(lua_State* L)
{
  ...
  double glossy = luaL_checknumber(L, 3);

  Material* material = self->m_material;
  material->setReflection(reflection, glossy);

  return 0;
}

// Set refraction property
extern "C"
int gr_node_set_refraction_cmd(lua_State* L)
{
  ...
  double glossy = luaL_checknumber(L, 4);
  Material* material = self->m_material;
  material->setRefraction(transparency, IOR, glossy);

  return 0;
}

// Set Perlin Noise Texture
extern "C"
int gr_node_set_perlin_noise_cmd(lua_State* L)
{
  ...
  int vertical = luaL_checknumber(L, 3);

  Material* material = self->m_material;
  Perlin* perlin = new Perlin();
  perlin->setVertical(static_cast<bool>(vertical));
  material->setPerlinNoise(perlin, scale);
...
// Set Illumination
extern "C"
int gr_node_set_illumination_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  AreaLight* light = ldata->light;
  Material* material = self->m_material;
  material->setIllumination(light);
...
// Set Image Texture
extern "C"
int gr_node_set_image_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  Material* material = self->m_material;
  if (self->m_primitive->isMeshObject()) {
    Mesh* mesh = dynamic_cast<Mesh*>(self->m_primitive);
    material->setImages(mesh->mat_img_map);
  } else {
    std::string filename = luaL_checkstring(L, 2);
    // int repeat = luaL_checknumber(L, 3);
...
static const luaL_Reg grlib_node_methods[] = {
  ...
  {"set_perlin", gr_node_set_perlin_noise_cmd},
  {"set_image", gr_node_set_image_cmd},
  {"set_illumination", gr_node_set_illumination_cmd},
