  ...
  std::cout << "F23: Calling A4_Render(\n" <<
	...
	for (uint y = 0; y < h; ++y) {
		for (uint x = 0; x < w; ++x) {
			...
			glm::vec3 color = getColor(rray, data, ambient, lights);
...
glm::vec3 getColor(Ray& ray, IntersectionData& data, const glm::vec3 & ambient, const std::list<Light *> & lights) {
	glm::vec3 color(1.0f);
	if (root->intersect(ray, data)) {
		...
		for (Light* light: lights) {
			...
			// diffuse
			color += glm::dot(lightDir, surfaceNorm) * attenuation * material->diffuse() * light->colour;

			// specular
			color += glm::pow(std::max(0.0, (double)glm::dot(reflection, viewDir)), material->shininess()) * attenuation * material->specular() * light->colour;
		}
	}
	return color;
}
