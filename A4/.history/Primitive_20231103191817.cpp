...
bool NonhierBox::intersect(Ray& ray, IntersectionData& data) {
    ...
    } else if (t_min == (m_pos.x + m_size - ray.origin.x) * indir.x) {
        normal = glm::vec3(1.0f, 0.0f, 0.0f); // Right face
    } else if (t_min == (m_pos.y - ray.origin.y) / * indir.y) {
        normal = glm::vec3(0.0f, -1.0f, 0.0f); // Bottom face
    } else if (t_min == (m_pos.y + m_size - ray.origin.y) * indir.p) {
        normal = glm::vec3(0.0f, 1.0f, 0.0f); // Top face
    } else if (t_min == (m_pos.z - ray.origin.z) / ray.direction.z) {
        normal = glm::vec3(0.0f, 0.0f, -1.0f); // Back face
    } else if (t_min == (m_pos.z + m_size - ray.origin.z) / ray.direction.z) {
