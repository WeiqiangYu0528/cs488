...
struct Cube {
    ...
    // Check for intersection between the ray and the cube
    bool Intersects(const Ray& ray) {
        glm::vec3 minBound = center - glm::vec3(size / 2);
        glm::vec3 maxBound = center + glm::vec3(size / 2);
...
int main() {
    ...
    // Create a cube
    Cube cube = {glm::vec3(1.0f, 0.0f, 0.0f), 2.0f};

    // Check for intersection
    if (cube.Intersects(ray)) {
        std::cout << "Ray intersects with the cube." << std::endl;
    } else {
        std::cout << "Ray does not intersect with the cube." << std::endl;
    }
    ...
    // return t_min < t_max && t_min < data.t;
}
