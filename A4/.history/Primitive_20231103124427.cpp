...
bool NonhierBox::intersect(Ray& ray, IntersectionData& data) {
    ...
    if (fabs(ray.direction.z < Epsilon)) {
        ...
        if (ray.direction.z >= 0) {
            t_zmin = (m_pos.z - ray.origin.z) / ray.direction.x;
            t_zmax = (m_pos.z + m_size - ray.origin.x) / ray.direction.x;
        } else {
            t_zmin = (m_pos.z + m_size - ray.origin.x) / ray.direction.x;
            t_zmax = (m_pos.z - ray.origin.x) / ray.direction.x;
        }
    }
}
