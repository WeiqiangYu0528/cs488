...
// 	glm::mat4 c2w(glm::vec4(u, 0.0), glm::vec4(v, 0.0), glm::vec4(w, 0.0), glm::vec4(eye, 1.0));
// 	float Px = (2 * ((x + 0.5) / wf) - 1) * tanf(glm::radians(fovy) / 2.0f);
// 	float Py = (1 - 2 * ((y + 0.5) / hf)) * tanf(glm::radians(fovy) / 2.0f);
// 	// glm::mat3 mat_inv = glm::transpose(mat);
// 	// for (int i = 0; i < 3; ++i) {
// 	// 	std::cout << mat_inv[i].x << " " << mat_inv[i].y << " " << mat_inv[i].z << std::endl;
// 	// }
...
// 	// glm::vec3 w_p = (-wf / 2.0 ) * mat[0] + (hf / 2.0) * mat[1] - ((hf / 2.0) / tanf(glm::radians(fovy) * 0.5)) * mat[2];
// 	glm::vec4 dir = c2w * glm::vec4(Px, Py, -1.0f, 1.0f);
// 	glm::vec3 direction = glm::normalize(glm::vec3(dir) - eye);
// 	std::cout << direction.x << " " << direction.y << " " << direction.z << std::endl;
// 	return direction;
// }
...
void getDirection(
	...
	glm::vec3& direction) {
		...
		glm::vec4 worldCoor = T4 * R3 * S * T1 * pixelCoor;
