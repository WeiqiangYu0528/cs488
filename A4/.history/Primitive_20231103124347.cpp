...
bool NonhierBox::intersect(Ray& ray, IntersectionData& data) {
    ...
    if (fabs(ray.direction.y < Epsilon)) {
        ...
        if (ray.direction.y >= 0) {
            t_ymin = (m_pos.y - ray.origin.y) / ray.direction.x;
            t_ymax = (m_pos.y + m_size - ray.origin.y) / ray.direction.x;
        } else {
            t_ymin = (m_pos.x + m_size - ray.origin.x) / ray.direction.x;
            t_ymax = (m_pos.x - ray.origin.x) / ray.direction.x;
        }
    }
