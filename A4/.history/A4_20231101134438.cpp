...
glm::vec3 getColor(SceneNode * root, Ray& ray, IntersectionData& data, const glm::vec3 & ambient, const std::list<Light *> & lights) {
	...
	if (root->intersect(ray, data)) {
		...
		for (Light* light: lights) {
			...
			// diffuse
			color += glm::dot(lightDir, surfaceNorm) * attenuation * material->getDiffuse() * light->colour;

			// specular
			color += glm::pow(std::max(0.0, (double)glm::dot(reflection, viewDir)), material->getShininess()) * attenuation * material->specular() * light->colour;
		}
	}
	return color;
}
