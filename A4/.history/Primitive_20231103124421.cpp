...
bool NonhierBox::intersect(Ray& ray, IntersectionData& data) {
    ...
    if (fabs(ray.direction.z < Epsilon)) {
        ...
        if (ray.direction.z >= 0) {
            t_zmin = (m_pos.x - ray.origin.x) / ray.direction.x;
            t_zmax = (m_pos.x + m_size - ray.origin.x) / ray.direction.x;
        } else {
            t_zmin = (m_pos.x + m_size - ray.origin.x) / ray.direction.x;
            t_zmax = (m_pos.x - ray.origin.x) / ray.direction.x;
        }
    }
}
