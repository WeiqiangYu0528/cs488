...
glm::vec3 getDirection(const glm::vec3& eye, const glm::vec3& view, const glm::vec3& up, double fovy,
	...
	glm::mat3 w2c (u, v, w);
	glm::mat3 c2w = glm::transpose(mat);
	glm::mat3 mat = glm::scale(glm::mat3(), (-1.0, 1.0, 1.0)) * glm::transpose(w2c) *

	glm::vec3 w_p = (-wf / 2.0 ) * mat[0] + (hf / 2.0) * mat[1] + ((hf / 2.0) / tanf(glm::radians(fovy) * 0.5)) * mat[2];

	glm::vec3 direction = glm::normalize(x * mat_inv[0] + y * (-mat_inv[1]) + w_p);
